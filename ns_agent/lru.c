...
void lruc_remove_item(lruc *cache, lruc_item *prev, lruc_item *item,
                      uint32_t hash_index) {
  if (prev)
    prev->next = item->next;
  else
...
#define test_for_missing_key()                                                 \
  error_for(!key || key_length == 0, LRUC_MISSING_KEY)
#define test_for_missing_value()                                               \
  error_for(!value || value_length == 0, LRUC_MISSING_VALUE)
...
lruc *lruc_new(uint64_t cache_size, uint32_t average_length) {
  ...
  cache->items =
      (lruc_item **)calloc(sizeof(lruc_item *), cache->hash_table_size);
  if (!cache->items) {
...
lruc_error lruc_set(lruc *cache, void *key, uint32_t key_length, void *value,
